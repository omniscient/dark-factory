...
sched_lib="$(cd "$(dirname "$0")" && pwd)/../scripts/scheduler_lib.sh"

# get_size_label must recognise XL (not just S/M/L) — moved to scripts/scheduler_lib.sh (#185)
grep -qE 'XL|xl' <(awk '/^get_size_label\(\)/{f=1} f{print} f&&/^}/{exit}' "$sched_lib") \
  || { echo "FAIL: get_size_label does not recognise XL"; exit 1; }

# is_above_ceiling must park XL (not L) — moved to scripts/scheduler_lib.sh (#185)
block="$(awk '/^is_above_ceiling\(\)/{f=1} f{print} f&&/^}/{exit}' "$sched_lib")"
...
# is_below_ceiling must treat L as below ceiling (timer-advance applies to S and L) — moved
# to scripts/scheduler_lib.sh (#185)
grep -qE 'S\|L\|""\)' <(awk '/^is_below_ceiling\(\)/{f=1} f{print} f&&/^}/{exit}' "$sched_lib") \
