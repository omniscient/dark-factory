#!/usr/bin/env python3
"""Factory Scorecard → scorecard.json
...
date window. Emits a ``scorecard.json`` structure consumed by downstream
reporting/analysis (e.g. the weekly dispatch-ceiling revisit).

A *factory PR* is any PR with ≥1 commit authored by the factory's git identity
(``FACTORY_EMAIL``, default ``factory@<repo>``) — all factory PRs share one
GitHub login, so commit authorship is the only reliable fingerprint. Co-authors
(``noreply@anthropic.com``) appear on both factory and human commits and must be
ignored.
"""
import json
import os
...
# Parity defaults match scripts/identity.sh (no env ⇒ MarketHawk behavior).
_DEFAULT_REPO = "omniscient/markethawk"
REPO = os.environ.get("FACTORY_REPO_SLUG") or _DEFAULT_REPO
# entrypoint.sh sets git author factory@<FACTORY_REPO>; derive the same default.
FACTORY_EMAIL = os.environ.get("FACTORY_EMAIL") or f"factory@{os.environ.get('FACTORY_REPO') or REPO.split('/')[-1]}"
REGRESSION_LABEL = os.environ.get("FACTORY_REGRESSION_LABEL", "factory-regression")
EDITS_LABEL = os.environ.get("FACTORY_EDITS_LABEL", "merged-with-edits")
...
def _fetch_pr_commits_rest(pr_number: int) -> list[dict]:
    ...
    ``FACTORY_EMAIL`` as the *primary* commit author, so omitting co-authors is
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--repo",
        default=None,
        help="owner/repo override (default: $FACTORY_REPO_SLUG or omniscient/markethawk)",
    )
    parser.add_argument(
        "--factory-email",
        default=None,
        help="factory git-author email override (default: $FACTORY_EMAIL or factory@<repo>)",
    )
    args = parser.parse_args()

    # Explicit CLI overrides win over env-resolved module globals. These names
    # are read inside module functions (fetch_prs/compute_churn/is_factory_*),
