#!/usr/bin/env python3
"""Dispatch ceiling keyword revisit analysis.

Wraps the co-located fetch_scorecard.py and applies the decision rules from the
dispatch-ceiling revisit design spec.

Outputs a Markdown report to stdout (or --output file). Does NOT post to
GitHub — that is handled by the Archon command (commands/ceiling-revisit.md).
"""
...
_PRODUCT = os.environ.get("FACTORY_PRODUCT_NAME", "MarketHawk")
...
def generate_report(
    ...
    lines += [
        "---",
        f"*Posted by {_PRODUCT} Weekly Ceiling Revisit*",
    ]
